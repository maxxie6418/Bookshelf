...
export function openSettings() {
  ...
  const themeToggle = h('button', {
    class: 'px-4 py-2 rounded-xl border border-shelf-200 dark:border-shelf-700 text-sm text-shelf-700 dark:text-shelf-200',
    onclick: () => toggleTheme(),
  ...
  const content = h('div', { class: 'space-y-5' },
    h('div', {},
      ...
      h('button', {
        class: 'mt-3 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm',
        onclick: async () => {
